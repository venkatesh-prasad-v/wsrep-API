...
#define VERIFY(_p)                                               \
  ...
  if (wsrep_check_iface_version(wh->version, iface_ver)) return EINVAL;
...
int wsrep_load(const char *spec, wsrep_t **hptr, wsrep_log_cb_t log_cb) {
  ...
  if (NULL != log_cb) logger = log_cb;

  if (!(spec && hptr)) return EINVAL;

  snprintf(msg, msg_len, "wsrep_load(): loading provider library '%s'", spec);
  ...
  if ((ret = (*dlfun)(*hptr)) != 0) {
    snprintf(msg, msg_len, "wsrep_load(): loader failed: %s", strerror(ret));
    logger(WSREP_LOG_ERROR, msg);
    goto out;
  }
  ...
  if ((ret = verify(*hptr, WSREP_INTERFACE_VERSION)) != 0) {
    snprintf(msg, msg_len,
             ...
             "provider version %s",
             WSREP_INTERFACE_VERSION, (*hptr)->version);
    logger(WSREP_LOG_ERROR, msg);
    goto out;
  }
...
out:
  if (ret != 0) {
    ...
    *hptr = NULL;
    ...
    snprintf(msg, msg_len, "wsrep_load(): %s %s by %s loaded successfully.",
...
void wsrep_unload(wsrep_t *hptr) {
  if (!hptr) {
    ...
    if (hptr->free) hptr->free(hptr);
    if (hptr->dlh) dlclose(hptr->dlh);
    free(hptr);
  }
}
