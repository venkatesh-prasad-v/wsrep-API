...
static void logger_cb(wsrep_log_level_t level __attribute__((unused)),
                      const char *msg) {
...
static wsrep_cb_status_t view_cb(void *app_ctx __attribute__((unused)),
  ...
  printf(
      "New cluster membership view: %d nodes, my index is %d, "
      "global seqno: %lld\n",
...
static wsrep_cb_status_t apply_cb(void *recv_ctx,
  ...
  snprintf(ctx->msg, sizeof(ctx->msg), "Got writeset %lld, size %zu",
           (long long)meta->gtid.seqno, ws_size);
 ...
 *  by seqno. */
static wsrep_cb_status_t commit_cb(void *recv_ctx,
...
static wsrep_cb_status_t sst_donate_cb(void *app_ctx __attribute__((unused)),
                                       void *recv_ctx __attribute__((unused)),
...
static void graceful_leave(int signum) {
...
int main(int argc, char *argv[]) {
  if (argc != 4) {
    fprintf(stderr,
            "Usage: %s </path/to/wsrep/provider> <wsrep URI> "
            "<cluster name>\n",
            argv[0]);
    exit(EXIT_FAILURE);
  }
  ...
  for (i = 0; i < num_threads; i++) {
    int err = pthread_create(&threads[i], NULL, recv_thread, &thread_ctx[i]);

    if (err) {
      fprintf(stderr, "Failed to start thread %d: %d (%s)", i, err,
              strerror(err));
      exit(EXIT_FAILURE);
    }
  }
