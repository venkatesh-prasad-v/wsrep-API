...
static wsrep_status_t dummy_options_set(wsrep_t *w, const char *conf) {
  WSREP_DBUG_ENTER(w);
  if (WSREP_DUMMY(w)->options) {
    free(WSREP_DUMMY(w)->options);
    WSREP_DUMMY(w)->options = NULL;
...
static char *dummy_options_get(wsrep_t *w) {
  WSREP_DBUG_ENTER(w);
  if (WSREP_DUMMY(w)->options) return strdup(WSREP_DUMMY(w)->options);
  return NULL;
}
...
static wsrep_status_t dummy_replicate(
    wsrep_t *w, const wsrep_conn_id_t conn_id __attribute__((unused)),
...
static wsrep_status_t dummy_interim_commit(wsrep_t *w,
...
static wsrep_status_t dummy_post_commit(wsrep_t *w, wsrep_ws_handle_t *ws_handle
                                        __attribute__((unused))) {
  WSREP_DBUG_ENTER(w);
  return WSREP_OK;
}

static wsrep_status_t dummy_applier_pre_commit(wsrep_t *w, void *trx_handle
                                               __attribute__((unused))) {
  WSREP_DBUG_ENTER(w);
  return WSREP_OK;
}

static wsrep_status_t dummy_applier_interim_commit(wsrep_t *w, void *trx_handle
                                                   __attribute__((unused))) {
  WSREP_DBUG_ENTER(w);
  return WSREP_OK;
}

static wsrep_status_t dummy_applier_post_commit(wsrep_t *w, void *trx_handle
                                                __attribute__((unused))) {
  WSREP_DBUG_ENTER(w);
  return WSREP_OK;
}

static wsrep_status_t dummy_post_rollback(wsrep_t *w,
                                          wsrep_ws_handle_t *ws_handle
                                          __attribute__((unused))) {
  WSREP_DBUG_ENTER(w);
  return WSREP_OK;
...
static wsrep_status_t dummy_replay_trx(wsrep_t *w,
                                       wsrep_ws_handle_t *ws_handle
                                       __attribute__((unused)),
...
static wsrep_status_t dummy_abort_pre_commit(wsrep_t *w,
...
static wsrep_status_t dummy_append_key(
    wsrep_t *w, wsrep_ws_handle_t *ws_handle __attribute__((unused)),
...
static wsrep_status_t dummy_causal_read(wsrep_t *w, wsrep_gtid_t *gtid
                                        __attribute__((unused))) {
  WSREP_DBUG_ENTER(w);
  return WSREP_OK;
}

static wsrep_status_t dummy_free_connection(wsrep_t *w,
...
static wsrep_status_t dummy_to_execute_end(wsrep_t *w,
                                           const wsrep_conn_id_t conn_id
                                           __attribute__((unused))) {
  WSREP_DBUG_ENTER(w);
  return WSREP_OK;
...
static wsrep_status_t dummy_preordered_collect(
    wsrep_t *w, wsrep_po_handle_t *handle __attribute__((unused)),
    const struct wsrep_buf *data __attribute__((unused)),
...
static wsrep_status_t dummy_sst_sent(wsrep_t *w,
...
static wsrep_status_t dummy_sst_received(
    wsrep_t *w, const wsrep_gtid_t *state_id __attribute__((unused)),
...
static wsrep_status_t dummy_snapshot(wsrep_t *w,
...
static struct wsrep_stats_var dummy_stats[] = {{NULL, WSREP_VAR_STRING, {0}}};
...
static void dummy_stats_free(wsrep_t *w, struct wsrep_stats_var *stats
                             __attribute__((unused))) {
  WSREP_DBUG_ENTER(w);
}

static void dummy_stats_reset(wsrep_t *w) { WSREP_DBUG_ENTER(w); }

static void dummy_fetch_pfs_info(wsrep_t *w,
...
static bool dummy_is_locked(wsrep_t *w, const char *s __attribute__((unused)),
                            uint64_t *o __attribute__((unused)),
...
static wsrep_t dummy_iface = {WSREP_INTERFACE_VERSION,
                              &dummy_init,
                              &dummy_capabilities,
                              &dummy_options_set,
                              &dummy_options_get,
...
int wsrep_dummy_loader(wsrep_t *w) {
  ...
  *w = dummy_iface;
  ...
  if (!(w->ctx = malloc(sizeof(wsrep_dummy_t)))) return ENOMEM;
